    ...
    try {
      ...
      if (!client) {
        ...
        const handleMissingDeviceIdentity = (): boolean => {
          if (!device) {
            // Shared token/password auth is gateway-level trust for operator clients;
            // preserve their self-declared scopes (consistent with skipPairingForOperatorSharedAuth).
            const isOperatorWithSharedAuth =
              role === "operator" && sharedAuthOk && !isControlUi && !isWebchat;
            if (!isOperatorWithSharedAuth) {
              clearUnboundScopes();
            }
          }
