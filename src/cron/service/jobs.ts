...
export function nextWakeAtMs(state: CronServiceState) {
  ...
  const enabled = jobs.filter((j) => j.enabled !== false && isFiniteTimestamp(j.state.nextRunAtMs));
  if (enabled.length === 0) {
    return undefined;
  }
...
export function isJobDue(job: CronJob, nowMs: number, opts: { forced: boolean }) {
  ...
  return job.enabled !== false && typeof job.state.nextRunAtMs === "number" && nowMs >= job.state.nextRunAtMs;
}
